...
auto Reader::get_num_cpus() -> std::size_t {
    ...
    /// Subtract 1 from the count to exclude the "cpu" line that corresponds to
    /// all CPUs.
    return cpu_count - 1;
}
...
auto Reader::read_data() -> void {
    ...
    while (!_exit_flag) {
        ...
        /// If the file cannot be opened, print an error message and wait before
        /// continuing.
        if (!file.is_open()) {
            _logger_buffer->push("Error: Could not open /proc/stat");
        ...
        /// Extract data for each CPU from the file and append it to a
        /// stringstream.
        while (std::getline(ss, line)) {
            /// Counts till the number of cores found in CPU
            if (line.find("cpu" + std::to_string(count)) == 0 &&
                count <= get_num_cpus()) {
        ...
        /// Convert the stringstream to a string and push it onto the analyzer
        /// buffer for processing.
        std::string raw_data = data_ss.str();
        _analyzer_buffer->push(raw_data);

        /// Close the file and wait for a fixed amount of time before reading
        /// again.
        file.close();
        std::unique_lock<std::mutex> lock(_mutex);
